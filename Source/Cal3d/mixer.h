...
public:
  ...
  bool isCycleActive(int id);
  bool blendCycle(int id, float weight, float delay);
  bool clearCycle(int id, float delay);
  bool executeAction(int id, float delayIn, float delayOut, float weightTarget = 1.0f, bool autoLock = false, bool extract_root_motion = false, bool extract_root_yaw = false, float start_time = 0.f);
  ...
  CalAnimationCycle* getCycleAnim(const std::string& name);
  CalAnimationAction* getActionAnim(const std::string& name);
  ...
  void setPlayBackwards(bool value) { m_play_backwards = value; }
  ...
  float getAndClearDeltaYaw() {
      float new_delta = root_yaw_delta;
      root_yaw_delta = 0.f;
      return new_delta;
  }
...
protected:
  ...
  float root_yaw_delta = 0.f;

  // For time reversal
  bool m_play_backwards = false;
