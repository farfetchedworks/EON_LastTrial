...
void CalAnimation::setAnimId(int id)
{
    anim_id = id;
}

int CalAnimation::getAnimId()
{
    return anim_id;
}
...
void CalAnimation::checkCallbacks(float animationTime,CalModel *model)
{
  ...
  for (size_t i=0; i<list.size(); i++)
  {
    ...
    list[i].callback->AnimationUpdate(animationTime, model, m_pCoreAnimation, model->getUserData());
...
void CalAnimation::completeCallbacks(CalModel *model)
{
  std::vector<CalCoreAnimation::CallbackRecord>& list = m_pCoreAnimation->getCallbackList();
  for (size_t i=0; i<list.size(); i++)
    list[i].callback->AnimationComplete(model, m_pCoreAnimation, model->getUserData());
}
