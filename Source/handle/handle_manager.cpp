...
bool            CHandleManager::any_handle_destroyed = false;

void CHandleManager::destroyAllPendingObjects() {
  if (!any_handle_destroyed)
    return;
...
CHandle CHandleManager::createHandle() {
  ...
  // El owner should have been deleted
  assert(ed.current_owner == CHandle());
...
void CHandleManager::onEntityCreated(CHandle who) {
  if (!who.isValid())
    return;
  auto& ed = external_to_internal[who.getExternalIndex()];
  onEntityCreatedObj(ed.internal_index);
}
