...
namespace ImGui {
  static bool Knob(const char* label, float* p_value, float v_min, float v_max, float default_value = 0, bool vertical = false)
  {
    std::string text = std::string(label);
    std::string result = text.substr(0, text.find("##"));
    ...
    ImGui::InvisibleButton(label, ImVec2(radius_outer * 2, radius_outer * 2 + ((result.size() != 0) ? line_height : 0) + style.ItemInnerSpacing.y));
    bool value_changed = false;
    bool is_active = ImGui::IsItemActive();
    bool is_hovered = ImGui::IsItemHovered();
    float delta = (vertical ? io.MouseDelta.y : io.MouseDelta.x);

    if (default_value && is_hovered && ImGui::IsMouseDoubleClicked(0)) {
      *p_value = default_value;
    }

    if (is_active && delta != 0.0f)
    {
      delta *= io.KeyShift ? .1f : 1.f;
